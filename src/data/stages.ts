...
let oseq = 0
function ob(element: Obstacle['element'], solids: Disc[]): Obstacle {
  return { id: `o${oseq++}`, element, solids, carves: [] }
}
/** 縦の柱：(cx, y0) から上へ n 個の円を積んだブロブ */
function pillar(cx: number, y0: number, n: number, element: Obstacle['element']): Obstacle {
  return ob(
    element,
 ...
 */
function wall(x0: number, x1: number, y0: number, rows: number, element: Obstacle['element']): Obstacle {
  const solids: Disc[] = []
...
const stage2: Stage = {
  ...
  // 山越え（放物線）で柱の上を越すか、削ってトンネルを開く。
  obstacles: [...colonnade(-18, 18, 3.6, -1, 4, ['dark', 'light'])],
  introText: [
