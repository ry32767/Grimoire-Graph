...
### 暴発の 2 段演出（`drawMisfire`）

**演出の最大半径＝効果範囲（`aoeRadius=5`）と一致させる**（#29）。`maxR = aoeRadius × scale` を基準にし、爆発の各要素（ヴォイド球・ドット・衝撃波リング）は `maxR` を超えない（`min(maxR, …)` でクランプ）。さらに **AoE 境界の破線円**を常時うっすら描き、ダメージが及ぶ範囲を明示する（見た目と実ダメージ範囲を揃える）。

- **収縮（progress 0〜0.3）**：22 粒が AoE 内から中心へ渦巻きながら吸い込まれ、闇のヴォイド球が育つ。
- **爆発（0.3〜1.0）**：ヴォイド球が `maxR`（=AoE 半径）まで膨張し、4 重リングのドット爆発が放射（紫ヴォイド／金／紫が交互）。**円形の衝撃波リング**が AoE 円に沿って広がり、効果範囲ちょうどで止まる。
