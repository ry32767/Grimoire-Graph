...
**白熱の中心から光（金）と闇（紫）が入り混じる多重衝撃波**を、**加算合成（`globalCompositeOperation='lighter'`）で超高コントラスト**に描く。演出は**効果範囲を少しはみ出して**弾ける（実効半径 `effR = aoeRadius × scale × 1.22`）。一方で**AoE 境界には白く明滅する破線円を `aoeRadius` ちょうどに描き**、実ダメージ範囲を明示する（爆発が少し外へ漏れて見えても、ダメージ範囲は円で分かる）。

- **収縮（progress 0〜0.22）**：光闇の粒（金/紫）が渦を巻いて中心へ吸い込まれ、白核が育つ。
- **爆発（0.22〜1.0）**：白熱コア（白→金→紫のグラデ）＋**時間差で広がる多重衝撃波**（白い芯＋光輪/闇輪が交互）が `effR` まで膨張。
- **光闇の粒が大量に飛散**：環状の粒（4 重・各 14〜32 個）に加え、**四方へバラついて散る光/闇/白の粒（52 個・重力つき）**。一部は AoE を越えて飛ぶ。
