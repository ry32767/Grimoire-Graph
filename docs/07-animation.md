...
**白熱の中心から光（金）と闇（紫）が入り混じる多重衝撃波**を、**加算合成（`globalCompositeOperation='lighter'`）で超高コントラスト**に描く。最大半径は効果範囲（`aoeRadius=5`）と一致（`maxR = aoeRadius × scale`、各要素 `min(maxR, …)` でクランプ）。**AoE 境界には白く明滅する破線円**を描きダメージ範囲を明示。

- **収縮（progress 0〜0.22）**：光闇の粒（金/紫）が渦を巻いて中心へ吸い込まれ、白核が育つ。
- **爆発（0.22〜1.0）**：白熱コア（白→金→紫のグラデ）＋**時間差で広がる多重衝撃波**（白い芯＋光輪/闇輪が交互）が `maxR` まで膨張。光闇＋白の粒が飛散（AoE 内）。
